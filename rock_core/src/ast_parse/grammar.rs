...
//@those get put into string lit intern pool, which will lead to them being generated
// and maybe discarded later, avoid that
fn attribute_param(p: &mut Parser) -> Result<AttrParam, String> {
    let name = name(p)?;
    ...
    let kind = match p.peek() {
        ...
        T![.] => {
            ...
            if p.at(T!['{']) {
                ...
                let name = name(p)?;
                let args_list = if p.at(T!['(']) {
                    Some(argument_list(p)?)
                } else {
                    None
                };
                ExprKind::Variant { name, args_list }
            }
        }
        T![ident] => {
            let path = path(p)?;
...
fn tail_expr<'ast>(
    ...
    loop {
        match p.peek() {
            ...
            T!['('] => {
                let args_list = argument_list(p)?;

                let expr = Expr {
                    kind: ExprKind::Call { target, args_list },
                    range: p.make_range(start),
                };
                target = p.state.arena.alloc(expr);
            }
...
fn lit(p: &mut Parser) -> Result<Lit, String> {
    match p.peek() {
        T![null] => {
            p.bump();
            Ok(Lit::Null)
        ...
        T![string_lit] => {
            ...
            let string_lit = StringLit { id, c_string };
            Ok(Lit::String(string_lit))
        }
        _ => unreachable!(),
    }
}
...
fn primary_pat<'ast>(p: &mut Parser<'ast, '_, '_>) -> Result<Pat<'ast>, String> {
    ...
    let kind = match p.peek() {
        ...
        T![ident] => {
            let path = path(p)?;
            let bind_list = if p.at(T!['(']) {
                Some(binding_list(p)?)
            } else {
                None
            };
            PatKind::Item { path, bind_list }
        }
        T![.] => {
            p.bump();
            let name = name(p)?;
            ...
            PatKind::Variant { name, bind_list }
        }
