...
fn resolve_import(ctx: &mut HirCtx, import: &ast::ImportItem) {
    ...
    for target in import.targets.iter().copied() {
        ...
        let module_name = target.import_path.last().copied().unwrap();
        ...
        for name in directory_names {
            match target_dir.find(ctx.session, name.id) {
                ModuleOrDirectory::None => {
                    ...
                    let pkg_name = ctx.name(source_package_name_id);
        ...
        let _ = import_module(ctx, &target, target_id, module_name);
...
fn import_module(
    ...
    let alias = check_symbol_rename(ctx, module_name, target.rename)?;
    ctx.scope.check_already_defined_global(alias, ctx.session, &ctx.registry, &mut ctx.emit)?;

    let origin_id = ctx.scope.origin;
    let was_used = !target.symbols.is_empty(); //dont warn if module import has symbols
